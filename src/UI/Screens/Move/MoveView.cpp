...
namespace UI
{
	...
	MoveView::MoveView(lv_obj_t* parent)
		...
		, m_layoutRowDsc{LV_GRID_FR(1), LV_GRID_FR(3), LV_GRID_CONTENT, LV_GRID_TEMPLATE_LAST}
							  ...
							  item->setJogCallback(
								  [this](size_t axis_index, size_t jog_index, void* user_data) {
									  m_presenter->moveAxisRelative(
										  axis_index, s_relMoveValues[jog_index], s_feedRates[s_currentFeedRateIndex]);
								  },
								  this);
							  item->setHomeCallback([this](size_t axis_index, void* user_data)
													{ m_presenter->homeAxis(axis_index); },
													this);
							  return item;
						  });
	}
