/*
 * LvLed.gen.h
 *
 *  AUTO-GENERATED: 2026-01-06T18:42:45 by scripts/generate_lvgl_wrappers.py
 *  LVGL version: 9.5.0-dev
 */

#pragma once

#include "UI/Components/LVGL/LvObj.h"

namespace UI
{
    // AUTO-GENERATED BY generate_lvgl_wrappers.py -- DO NOT EDIT
    class LvLed;

    template <typename Derived>
    class LvLedMethodsGen
    {
      public:
        /**
         * Set the color of the LED
         * @param led       pointer to a LED object
         * @param color     the color of the LED
         */
        void setColor(lv_color_t color) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_led_set_color(static_cast<Derived*>(this)->getRootPtr(), color);
        }

        /**
         * Set the brightness of a LED object
         * @param led       pointer to a LED object
         * @param bright    LV_LED_BRIGHT_MIN (max. dark) ... LV_LED_BRIGHT_MAX (max. light)
         */
        void setBrightness(uint8_t bright) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_led_set_brightness(static_cast<Derived*>(this)->getRootPtr(), bright);
        }

        /**
         * Light on a LED
         * @param led       pointer to a LED object
         */
        void on() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_led_on(static_cast<Derived*>(this)->getRootPtr());
        }

        /**
         * Light off a LED
         * @param led       pointer to a LED object
         */
        void off() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_led_off(static_cast<Derived*>(this)->getRootPtr());
        }

        /**
         * Toggle the state of a LED
         * @param led       pointer to a LED object
         */
        void toggle() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_led_toggle(static_cast<Derived*>(this)->getRootPtr());
        }

        /**
         * Get the brightness of a LED object
         * @return bright   0 (max. dark) ... 255 (max. light)
         */
        uint8_t getBrightness() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_led_get_brightness(static_cast<const Derived*>(this)->getRootPtr());
        }

    };

    class LvLedGen : public LvObj, public LvLedMethodsGen<LvLed>
    {
      public:
        LvLedGen(const std::string& name, LvObj& parent)
            : LvObj(lv_led_create, name, parent)
        {
            UI_LOCK();
        }
    };
} // namespace UI
