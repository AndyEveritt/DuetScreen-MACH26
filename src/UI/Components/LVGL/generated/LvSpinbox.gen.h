/*
 * LvSpinbox.gen.h
 *
 *  AUTO-GENERATED: 2026-01-06T18:42:45 by scripts/generate_lvgl_wrappers.py
 *  LVGL version: 9.5.0-dev
 */

#pragma once

#include "UI/Components/LVGL/LvObj.h"

#include "UI/Components/LVGL/generated/LvTextarea.gen.h"

namespace UI
{
    // AUTO-GENERATED BY generate_lvgl_wrappers.py -- DO NOT EDIT

    #if LV_USE_SPINBOX

    class LvSpinbox;

    template <typename Derived>
    class LvSpinboxMethodsGen : public LvTextareaMethodsGen<Derived>
    {
      public:
        /**
         * Set spinbox value
         * @param v     value to be set
         */
        void setValue(int32_t v) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_value(static_cast<Derived*>(this)->getRootPtr(), v);
        }

        /**
         * Set spinbox rollover function
         * @param rollover  true or false to enable or disable (default)
         */
        void setRollover(bool rollover) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_rollover(static_cast<Derived*>(this)->getRootPtr(), rollover);
        }

        /**
         * Set spinbox digit format (digit count and decimal format)
         * @param digit_count   number of digit excluding the decimal separator and the sign
         * @param sep_pos       number of digit before the decimal point. If 0, decimal point is not
         * shown
         */
        void setDigitFormat(uint32_t digit_count, uint32_t sep_pos) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_digit_format(static_cast<Derived*>(this)->getRootPtr(), digit_count, sep_pos);
        }

        /**
         * Set the number of digits
         * @param digit_count   number of digits
         */
        void setDigitCount(uint32_t digit_count) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_digit_count(static_cast<Derived*>(this)->getRootPtr(), digit_count);
        }

        /**
         * Set the position of the decimal point
         * @param dec_point_pos 0: there is no separator, 2: two integer digits
         */
        void setDecPointPos(uint32_t dec_point_pos) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_dec_point_pos(static_cast<Derived*>(this)->getRootPtr(), dec_point_pos);
        }

        /**
         * Set spinbox step
         * @param step  steps on increment/decrement. Can be 1, 10, 100, 1000, etc the digit that will change.
         */
        void setStep(uint32_t step) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_step(static_cast<Derived*>(this)->getRootPtr(), step);
        }

        /**
         * Set spinbox value range
         * @param min_value minimum value, inclusive
         * @param max_value maximum value, inclusive
         */
        void setRange(int32_t min_value, int32_t max_value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_range(static_cast<Derived*>(this)->getRootPtr(), min_value, max_value);
        }

        /**
         * Set the minimum value
         * @param min_value the minimum value
         */
        void setMinValue(int32_t min_value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_min_value(static_cast<Derived*>(this)->getRootPtr(), min_value);
        }

        /**
         * Set the maximum value
         * @param max_value the maximum value
         */
        void setMaxValue(int32_t max_value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_max_value(static_cast<Derived*>(this)->getRootPtr(), max_value);
        }

        /**
         * Set cursor position to a specific digit for edition
         * @param pos   selected position in spinbox
         */
        void setCursorPos(uint32_t pos) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_cursor_pos(static_cast<Derived*>(this)->getRootPtr(), pos);
        }

        /**
         * Set direction of digit step when clicking an encoder button while in editing mode
         * @param direction     the direction (LV_DIR_RIGHT or LV_DIR_LEFT)
         */
        void setDigitStepDirection(lv_dir_t direction) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_set_digit_step_direction(static_cast<Derived*>(this)->getRootPtr(), direction);
        }

        /**
         * Get spinbox rollover function status
         */
        bool getRollover() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_spinbox_get_rollover(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the spinbox numeral value (user has to convert to float according to its digit format)
         * @return      value integer value of the spinbox
         */
        int32_t getValue() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_spinbox_get_value(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the spinbox step value (user has to convert to float according to its digit format)
         * @return      value integer step value of the spinbox
         */
        int32_t getStep() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_spinbox_get_step(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Select next lower digit for edition by dividing the step by 10
         */
        void stepNext() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_step_next(static_cast<Derived*>(this)->getRootPtr());
        }

        /**
         * Select next higher digit for edition by multiplying the step by 10
         */
        void stepPrev() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_step_prev(static_cast<Derived*>(this)->getRootPtr());
        }

        /**
         * Increment spinbox value by one step
         */
        void increment() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_increment(static_cast<Derived*>(this)->getRootPtr());
        }

        /**
         * Decrement spinbox value by one step
         */
        void decrement() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_spinbox_decrement(static_cast<Derived*>(this)->getRootPtr());
        }

        #if LV_USE_OBSERVER

        /**
         * Bind an integer subject to a Spinbox's value.
         * @param subject   pointer to Subject
         * @return          pointer to newly-created Observer
         */
        lv_observer_t * bindValue(lv_subject_t * subject) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_spinbox_bind_value(static_cast<Derived*>(this)->getRootPtr(), subject);
        }

        #endif
    };

    class LvSpinboxGen : public LvObj, public LvSpinboxMethodsGen<LvSpinbox>
    {
      public:
        LvSpinboxGen(const std::string& name, LvObj& parent)
            : LvObj(lv_spinbox_create, name, parent)
        {
            UI_LOCK();
        }
    };

    #endif

} // namespace UI
