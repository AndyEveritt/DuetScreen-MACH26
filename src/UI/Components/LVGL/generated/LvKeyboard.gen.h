/*
 * LvKeyboard.gen.h
 *
 *  AUTO-GENERATED: 2026-01-06T18:42:45 by scripts/generate_lvgl_wrappers.py
 *  LVGL version: 9.5.0-dev
 */

#pragma once

#include "UI/Components/LVGL/LvObj.h"
#include <span>

#include "UI/Components/LVGL/generated/LvButtonmatrix.gen.h"

namespace UI
{
    // AUTO-GENERATED BY generate_lvgl_wrappers.py -- DO NOT EDIT

    #if LV_USE_KEYBOARD

    class LvKeyboard;

    template <typename Derived>
    class LvKeyboardMethodsGen : public LvButtonmatrixMethodsGen<Derived>
    {
      public:
        /**
         * Assign a text area to the keyboard. Pressed characters will be inserted there.
         * @param kb        pointer to a keyboard object
         * @param ta        pointer to a text area object to write into
         */
        void setTextarea(lv_obj_t * ta) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_keyboard_set_textarea(static_cast<Derived*>(this)->getRootPtr(), ta);
        }

        /**
         * Set a new mode (e.g., text, number, special characters).
         * @param kb        pointer to a keyboard object
         * @param mode      the desired mode (see 'lv_keyboard_mode_t')
         */
        void setMode(lv_keyboard_mode_t mode) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_keyboard_set_mode(static_cast<Derived*>(this)->getRootPtr(), mode);
        }

        /**
         * Enable or disable popovers showing button titles on press.
         * @param kb        pointer to a keyboard object
         * @param en        true to enable popovers; false to disable
         */
        void setPopovers(bool en) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_keyboard_set_popovers(static_cast<Derived*>(this)->getRootPtr(), en);
        }

        /**
         * Set a custom button map for the keyboard.
         * @param kb        pointer to a keyboard object
         * @param mode      the mode to assign the new map to (see 'lv_keyboard_mode_t')
         * @param map       pointer to a string array describing the button map
         *                  see 'lv_buttonmatrix_set_map()' for more details
         * @param ctrl_map  pointer to the control map. See 'lv_buttonmatrix_set_ctrl_map()'
        
         */
        void setMap(lv_keyboard_mode_t mode, std::span<const char * const> map, std::span<const lv_buttonmatrix_ctrl_t> ctrl_map) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_keyboard_set_map(static_cast<Derived*>(this)->getRootPtr(), mode, map.data(), ctrl_map.data());
        }

        /**
         * Get the text area currently assigned to the keyboard.
         * @param kb        pointer to a keyboard object
         * @return          pointer to the assigned text area object
         */
        lv_obj_t * getTextarea() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_keyboard_get_textarea(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the current mode of the keyboard.
         * @param kb        pointer to a keyboard object
         * @return          the current mode (see 'lv_keyboard_mode_t')
         */
        lv_keyboard_mode_t getMode() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_keyboard_get_mode(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Check whether popovers are enabled on the keyboard.
         * @return          true if popovers are enabled; false otherwise
         */
        bool getPopovers() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_keyboard_get_popovers(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the current button map of the keyboard.
         * @param kb        pointer to a keyboard object
         * @return          pointer to the map array
         */
        const char * const * getMapArray() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_keyboard_get_map_array(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the index of the last selected button (pressed, released, focused, etc.).
         * Useful in the `event_cb` to retrieve button text or properties.
         * @return          index of the last interacted button
         *                  returns LV_BUTTONMATRIX_BUTTON_NONE if not set
         */
        uint32_t getSelectedButton() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_keyboard_get_selected_button(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the text of a button by index.
         * @param btn_id    index of the button (excluding newline characters)
         * @return          pointer to the text of the button
         */
        const char * getButtonText(uint32_t btn_id) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_keyboard_get_button_text(static_cast<const Derived*>(this)->getRootPtr(), btn_id);
        }

    };

    class LvKeyboardGen : public LvObj, public LvKeyboardMethodsGen<LvKeyboard>
    {
      public:
        LvKeyboardGen(const std::string& name, LvObj& parent)
            : LvObj(lv_keyboard_create, name, parent)
        {
            UI_LOCK();
        }
    };

    #endif

} // namespace UI
