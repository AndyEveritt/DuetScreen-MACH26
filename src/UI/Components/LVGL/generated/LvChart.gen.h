/*
 * LvChart.gen.h
 *
 *  AUTO-GENERATED: 2026-01-06T18:42:45 by scripts/generate_lvgl_wrappers.py
 *  LVGL version: 9.5.0-dev
 */

#pragma once

#include "UI/Components/LVGL/LvObj.h"
#include <span>

namespace UI
{
    // AUTO-GENERATED BY generate_lvgl_wrappers.py -- DO NOT EDIT

    #if LV_USE_CHART

    class LvChart;

    template <typename Derived>
    class LvChartMethodsGen
    {
      public:
        /**
         * Set a new type for a chart
         * @param type      new type of the chart (from 'lv_chart_type_t' enum)
         */
        void setType(lv_chart_type_t type) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_type(static_cast<Derived*>(this)->getRootPtr(), type);
        }

        /**
         * Set the number of points on a data line on a chart
         * @param cnt       new number of points on the data lines
         */
        void setPointCount(uint32_t cnt) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_point_count(static_cast<Derived*>(this)->getRootPtr(), cnt);
        }

        /**
         * Set the minimal and maximal y values on an axis
         * @param axis      `LV_CHART_AXIS_PRIMARY_Y` or `LV_CHART_AXIS_SECONDARY_Y`
         * @param min       minimum value of the y axis
         * @param max       maximum value of the y axis
         */
        void setAxisRange(lv_chart_axis_t axis, int32_t min, int32_t max) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_axis_range(static_cast<Derived*>(this)->getRootPtr(), axis, min, max);
        }

        /**
         * Set the minimal values on an axis
         * @param axis      `LV_CHART_AXIS_PRIMARY_Y` or `LV_CHART_AXIS_SECONDARY_Y`
         * @param min       minimal value of the y axis
         */
        void setAxisMinValue(lv_chart_axis_t axis, int32_t min) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_axis_min_value(static_cast<Derived*>(this)->getRootPtr(), axis, min);
        }

        /**
         * Set the maximal y values on an axis
         * @param axis      `LV_CHART_AXIS_PRIMARY_Y` or `LV_CHART_AXIS_SECONDARY_Y`
         * @param max       maximum value of the y axis
         */
        void setAxisMaxValue(lv_chart_axis_t axis, int32_t max) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_axis_max_value(static_cast<Derived*>(this)->getRootPtr(), axis, max);
        }

        /**
         * Set update mode of the chart object. Affects
         * @param update_mode      the update mode
         */
        void setUpdateMode(lv_chart_update_mode_t update_mode) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_update_mode(static_cast<Derived*>(this)->getRootPtr(), update_mode);
        }

        /**
         * Set the number of horizontal and vertical division lines
         * @param hdiv      number of horizontal division lines
         * @param vdiv      number of vertical division lines
         */
        void setDivLineCount(uint32_t hdiv, uint32_t vdiv) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_div_line_count(static_cast<Derived*>(this)->getRootPtr(), hdiv, vdiv);
        }

        /**
         * Set the number of horizontal division lines
         * @param cnt       number of horizontal division lines
         */
        void setHorDivLineCount(uint32_t cnt) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_hor_div_line_count(static_cast<Derived*>(this)->getRootPtr(), cnt);
        }

        /**
         * Set the number of vertical division lines
         * @param cnt       number of vertical division lines
         */
        void setVerDivLineCount(uint32_t cnt) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_ver_div_line_count(static_cast<Derived*>(this)->getRootPtr(), cnt);
        }

        /**
         * Get the type of a chart
         * @return          type of the chart (from 'lv_chart_t' enum)
         */
        lv_chart_type_t getType() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_type(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the data point number per data line on chart
         * @return          point number on each data line
         */
        uint32_t getPointCount() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_point_count(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the current index of the x-axis start point in the data array
         * @param ser       pointer to a data series on 'chart'
         * @return          the index of the current x start point in the data array
         */
        uint32_t getXStartPoint(lv_chart_series_t * ser) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_x_start_point(static_cast<const Derived*>(this)->getRootPtr(), ser);
        }

        /**
         * Get the position of a point to the chart.
         * @param ser       pointer to series
         * @param id        the index.
         * @param p_out     store the result position here
         */
        void getPointPosById(lv_chart_series_t * ser, uint32_t id, lv_point_t * p_out) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_get_point_pos_by_id(static_cast<const Derived*>(this)->getRootPtr(), ser, id, p_out);
        }

        /**
         * Refresh a chart if its data line has changed
         */
        void refresh() requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_refresh(static_cast<Derived*>(this)->getRootPtr());
        }

        /**
         * Allocate and add a data series to the chart
         * @param color     color of the data series
         * @param axis      the y axis to which the series should be attached (::LV_CHART_AXIS_PRIMARY_Y or ::LV_CHART_AXIS_SECONDARY_Y)
         * @return          pointer to the allocated data series or NULL on failure
         */
        lv_chart_series_t * addSeries(lv_color_t color, lv_chart_axis_t axis) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_add_series(static_cast<Derived*>(this)->getRootPtr(), color, axis);
        }

        /**
         * Deallocate and remove a data series from a chart
         * @param series    pointer to a data series on 'chart'
         */
        void removeSeries(lv_chart_series_t * series) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_remove_series(static_cast<Derived*>(this)->getRootPtr(), series);
        }

        /**
         * Hide/Unhide a single series of a chart.
         * @param chart     pointer to a chart object.
         * @param series    pointer to a series object
         * @param hide      true: hide the series
         */
        void hideSeries(lv_chart_series_t * series, bool hide) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_hide_series(static_cast<Derived*>(this)->getRootPtr(), series, hide);
        }

        /**
         * Change the color of a series
         * @param chart     pointer to a chart object.
         * @param series    pointer to a series object
         * @param color     the new color of the series
         */
        void setSeriesColor(lv_chart_series_t * series, lv_color_t color) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_series_color(static_cast<Derived*>(this)->getRootPtr(), series, color);
        }

        /**
         * Get the color of a series
         * @param chart     pointer to a chart object.
         * @param series    pointer to a series object
         * @return          the color of the series
         */
        lv_color_t getSeriesColor(const lv_chart_series_t * series) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_series_color(static_cast<const Derived*>(this)->getRootPtr(), series);
        }

        /**
         * Set the index of the x-axis start point in the data array.
         * This point will be considers the first (left) point and the other points will be drawn after it.
         * @param ser       pointer to a data series on 'chart'
         * @param id        the index of the x point in the data array
         */
        void setXStartPoint(lv_chart_series_t * ser, uint32_t id) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_x_start_point(static_cast<Derived*>(this)->getRootPtr(), ser, id);
        }

        /**
         * Get the next series.
         * @param chart     pointer to a chart
         * @param ser      the previous series or NULL to get the first
         * @return          the next series or NULL if there is no more.
         */
        lv_chart_series_t * getSeriesNext(const lv_chart_series_t * ser) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_series_next(static_cast<const Derived*>(this)->getRootPtr(), ser);
        }

        /**
         * Add a cursor with a given color
         * @param color     color of the cursor
         * @param dir       direction of the cursor. `LV_DIR_RIGHT/LEFT/TOP/DOWN/HOR/VER/ALL`. OR-ed values are possible
         * @return          pointer to the created cursor
         */
        lv_chart_cursor_t * addCursor(lv_color_t color, lv_dir_t dir) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_add_cursor(static_cast<Derived*>(this)->getRootPtr(), color, dir);
        }

        /**
         * Remove a cursor
         * @param cursor    pointer to the cursor
         */
        void removeCursor(lv_chart_cursor_t * cursor) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_remove_cursor(static_cast<Derived*>(this)->getRootPtr(), cursor);
        }

        /**
         * Set the coordinate of the cursor with respect to the paddings
         * @param chart     pointer to a chart object
         * @param cursor    pointer to the cursor
         * @param pos       the new coordinate of cursor relative to the chart
         */
        void setCursorPos(lv_chart_cursor_t * cursor, lv_point_t * pos) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_cursor_pos(static_cast<Derived*>(this)->getRootPtr(), cursor, pos);
        }

        /**
         * Set the X coordinate of the cursor with respect to the paddings
         * @param chart     pointer to a chart object
         * @param cursor    pointer to the cursor
         * @param x         the new X coordinate of cursor relative to the chart
         */
        void setCursorPosX(lv_chart_cursor_t * cursor, int32_t x) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_cursor_pos_x(static_cast<Derived*>(this)->getRootPtr(), cursor, x);
        }

        /**
         * Set the coordinate of the cursor with respect to the paddings
         * @param chart     pointer to a chart object
         * @param cursor    pointer to the cursor
         * @param y         the new Y coordinate of cursor relative to the chart
         */
        void setCursorPosY(lv_chart_cursor_t * cursor, int32_t y) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_cursor_pos_y(static_cast<Derived*>(this)->getRootPtr(), cursor, y);
        }

        /**
         * Stick the cursor to a point
         * @param chart     pointer to a chart object
         * @param cursor    pointer to the cursor
         * @param ser       pointer to a series
         * @param point_id  the point's index or `LV_CHART_POINT_NONE` to not assign to any points.
         */
        void setCursorPoint(lv_chart_cursor_t * cursor, lv_chart_series_t * ser, uint32_t point_id) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_cursor_point(static_cast<Derived*>(this)->getRootPtr(), cursor, ser, point_id);
        }

        /**
         * Get the coordinate of the cursor with respect to the paddings
         * @param chart     pointer to a chart object
         * @param cursor    pointer to cursor
         * @return          coordinate of the cursor as lv_point_t
         */
        lv_point_t getCursorPoint(lv_chart_cursor_t * cursor) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_cursor_point(static_cast<const Derived*>(this)->getRootPtr(), cursor);
        }

        /**
         * Initialize all data points of a series with a value
         * @param ser       pointer to a data series on 'chart'
         * @param value     the new value for all points. `LV_CHART_POINT_NONE` can be used to hide the points.
         */
        void setAllValues(lv_chart_series_t * ser, int32_t value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_all_values(static_cast<Derived*>(this)->getRootPtr(), ser, value);
        }

        /**
         * Set the next point's Y value according to the update mode policy.
         * @param ser       pointer to a data series on 'chart'
         * @param value     the new value of the next data
         */
        void setNextValue(lv_chart_series_t * ser, int32_t value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_next_value(static_cast<Derived*>(this)->getRootPtr(), ser, value);
        }

        /**
         * Set the next point's X and Y value according to the update mode policy.
         * @param ser       pointer to a data series on 'chart'
         * @param x_value   the new X value of the next data
         * @param y_value   the new Y value of the next data
         */
        void setNextValue2(lv_chart_series_t * ser, int32_t x_value, int32_t y_value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_next_value2(static_cast<Derived*>(this)->getRootPtr(), ser, x_value, y_value);
        }

        /**
         * Same as `lv_chart_set_next_value` but set the values from an array
         * @param ser           pointer to a data series on 'chart'
         * @param values        the new values to set
         * @param values_cnt    number of items in `values`
         */
        void setSeriesValues(lv_chart_series_t * ser, std::span<const int32_t> values) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_series_values(static_cast<Derived*>(this)->getRootPtr(), ser, values.data(), values.size());
        }

        /**
         * Same as `lv_chart_set_next_value2` but set the values from an array
         * @param ser           pointer to a data series on 'chart'
         * @param x_values      the new values to set on the X axis
         * @param y_values      the new values to set o nthe Y axis
         * @param values_cnt    number of items in `x_values` and `y_values`
         */
        void setSeriesValues2(lv_chart_series_t * ser, std::span<const int32_t> x_values, std::span<const int32_t> y_values) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_series_values2(static_cast<Derived*>(this)->getRootPtr(), ser, x_values.data(), y_values.data(), y_values.size());
        }

        /**
         * Set an individual point's y value of a chart's series directly based on its index
         * @param ser     pointer to a data series on 'chart'
         * @param id      the index of the x point in the array
         * @param value   value to assign to array point
         */
        void setSeriesValueById(lv_chart_series_t * ser, uint32_t id, int32_t value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_series_value_by_id(static_cast<Derived*>(this)->getRootPtr(), ser, id, value);
        }

        /**
         * Set an individual point's x and y value of a chart's series directly based on its index
         * Can be used only with `LV_CHART_TYPE_SCATTER`.
         * @param ser       pointer to a data series on 'chart'
         * @param id        the index of the x point in the array
         * @param x_value   the new X value of the next data
         * @param y_value   the new Y value of the next data
         */
        void setSeriesValueById2(lv_chart_series_t * ser, uint32_t id, int32_t x_value, int32_t y_value) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_series_value_by_id2(static_cast<Derived*>(this)->getRootPtr(), ser, id, x_value, y_value);
        }

        /**
         * Set an external array for the y data points to use for the chart
         * NOTE: It is the users responsibility to make sure the `point_cnt` matches the external array size.
         * @param ser       pointer to a data series on 'chart'
         * @param array     external array of points for chart
         */
        void setSeriesExtYArray(lv_chart_series_t * ser, std::span<int32_t> array) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_series_ext_y_array(static_cast<Derived*>(this)->getRootPtr(), ser, array.data());
        }

        /**
         * Set an external array for the x data points to use for the chart
         * NOTE: It is the users responsibility to make sure the `point_cnt` matches the external array size.
         * @param ser       pointer to a data series on 'chart'
         * @param array     external array of points for chart
         */
        void setSeriesExtXArray(lv_chart_series_t * ser, std::span<int32_t> array) requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            lv_chart_set_series_ext_x_array(static_cast<Derived*>(this)->getRootPtr(), ser, array.data());
        }

        /**
         * Get the array of y values of a series
         * @param ser   pointer to a data series on 'chart'
         * @return      the array of values with 'point_count' elements
         */
        int32_t * getSeriesYArray(lv_chart_series_t * ser) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_series_y_array(static_cast<const Derived*>(this)->getRootPtr(), ser);
        }

        /**
         * Get the array of x values of a series
         * @param ser   pointer to a data series on 'chart'
         * @return      the array of values with 'point_count' elements
         */
        int32_t * getSeriesXArray(lv_chart_series_t * ser) const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_series_x_array(static_cast<const Derived*>(this)->getRootPtr(), ser);
        }

        /**
         * Get the index of the currently pressed point. It's the same for every series.
         * @return          the index of the point [0 .. point count] or LV_CHART_POINT_ID_NONE if no point is being pressed
         */
        uint32_t getPressedPoint() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_pressed_point(static_cast<const Derived*>(this)->getRootPtr());
        }

        /**
         * Get the overall offset from the chart's side to the center of the first point.
         * In case of a bar chart it will be the center of the first column group
         * @return          the offset of the center
         */
        int32_t getFirstPointCenterOffset() const requires HasGetRootPtr<Derived>
        {
            UI_LOCK();
            return lv_chart_get_first_point_center_offset(static_cast<const Derived*>(this)->getRootPtr());
        }

    };

    class LvChartGen : public LvObj, public LvChartMethodsGen<LvChart>
    {
      public:
        LvChartGen(const std::string& name, LvObj& parent)
            : LvObj(lv_chart_create, name, parent)
        {
            UI_LOCK();
        }
    };

    #endif

} // namespace UI
