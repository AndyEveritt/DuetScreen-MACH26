...
namespace OM
{
	...
	static Debug::DebugCommand s_logOM(
		...
		[]()
		{
			...
			for (size_t i = 0; i < MAX_HEATERS; ++i)
			{
				...
				if (heater != nullptr)
				{
					printf("    [%u]: status(%d), active(%d), standby(%d), current(%.2f),",
						   ...
						   heater->standbyTemp,
						   heater->current);
					printf("             min(%.1f), max(%.1f), pwm(%.2f), sensor(%d)",
						   heater->min,
						   heater->max,
						   heater->avgPwm,
			...
			printf("  Printer Name: %s", OM::GetPrinterName().c_str());
			printf("  Printer State: %s(%d)", OM::GetStatusText(), (int)OM::GetStatus());
