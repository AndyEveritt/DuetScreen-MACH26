...
void SubscriberMap::_addSubscriber(const char* key, subscriberCb_t cb)
{
	...
	LOG_VERBOSE("Adding subscriber for key '{:s}' cb @ {}", key, static_cast<const void*>(&cb));
										  ...
										  bool (*cb)(Comm::JsonDecoder* decoder, const size_t indices[]))
{
	...
	if (key[strlen(key) - 1] != '^')
	{
		LOG_ERROR("Key '{:s}' must end with '^' as it is provided as an array end subscriber", key);
		return;
	}
