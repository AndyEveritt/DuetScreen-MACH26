...
class String
{
  public:
	...
	bool copy(const char* src, size_t maxlen) noexcept
	{
		return GetRef().copy(src, maxlen);
	} // returns true if buffer is too small
	bool cat(const char* src) noexcept { return GetRef().cat(src); } // returns true if buffer is too small
	bool catn(const char* src, size_t n) noexcept
	{
		return GetRef().catn(src, n);
	} // returns true if buffer is too small
	...
	bool Insert(size_t pos, const char* s) noexcept
	{
		return GetRef().Insert(pos, s);
	} // returns true if buffer is too small
	...
	// Compare with a C string. If the C string is too long but the part of it we could accommodate matches, return
	// true. this will ignore the \0 at the end
...
template <size_t Len>
void String<Len>::CopyAndPad(const char* src) noexcept
{
...
template <size_t Len>
bool String<Len>::ConstantTimeEquals(String<Len> other) const noexcept
{
	uint8_t rslt = 0;
...
template <size_t Len>
inline int String<Len>::vprintf(const char* fmt, va_list vargs) noexcept
{
	return GetRef().vprintf(fmt, vargs);
}

template <size_t Len>
inline int String<Len>::vcatf(const char* fmt, va_list vargs) noexcept
{
	return GetRef().vcatf(fmt, vargs);
}

template <size_t Len>
inline bool String<Len>::Prepend(const char* src) noexcept
{
	return GetRef().Prepend(src);
}

template <size_t Len>
int String<Len>::printf(const char* fmt, ...) noexcept
{
	va_list vargs;
	va_start(vargs, fmt);
...
template <size_t Len>
int String<Len>::catf(const char* fmt, ...) noexcept
{
	va_list vargs;
	va_start(vargs, fmt);
...
template <size_t Len>
void String<Len>::Truncate(size_t len) noexcept
{
	if (len < Len)
	{
		storage[len] = 0;
...
template <size_t Len>
void String<Len>::Erase(size_t pos, size_t count) noexcept
{
...
template <size_t Len>
bool String<Len>::EndsWith(char c) const noexcept
{
	const size_t len = strlen();
...
template <size_t Len>
int String<Len>::Contains(const char* s) const noexcept
{
...
template <size_t Len>
int String<Len>::Contains(char c) const noexcept
{
	const char* const p = strchr(storage, (int)c);
	return (p == nullptr) ? -1 : p - storage;
}
