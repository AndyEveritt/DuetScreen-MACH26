...
class function_ref<RetType(Params...)>
{
	...
	static RetType callback_fn(void* callable2, Params... params)
	{
  ...
  public:
	...
	RetType operator()(Params... params) const { return callback(callable, std::forward<Params>(params)...); }
};
...
class function_ref_noexcept<RetType(Params...) noexcept>
{
	...
	template <typename Callable>
	static RetType callback_fn(void* callable2, Params... params) noexcept
	{
		return (*reinterpret_cast<Callable*>(callable2))(std::forward<Params>(params)...);
	}
  ...
  public:
	...
	function_ref_noexcept(
		Callable&& callable2,
		typename std::enable_if<!std::is_same<typename std::remove_reference<Callable>::type,
											  function_ref_noexcept>::value>::type* = nullptr) noexcept
		: callback(callback_fn<typename std::remove_reference<Callable>::type>)
		, callable(reinterpret_cast<void*>(&callable2))
	{
	}
	...
	/// Call the stored callable with the given arguments
	RetType operator()(Params... params) const noexcept { return callback(callable, std::forward<Params>(params)...); }
};
